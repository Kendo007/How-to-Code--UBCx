...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname Score-space-invaders) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(check-random (next-game G1)
              (make-game empty empty (make-tank (+ 50 TANK-SPEED) 1) 0))
...
(define (next-game g)
  (make-game (next-invaders (game-invaders g) (game-missiles g))
             ...
             (next-score (game-score g) (game-invaders g) (game-missiles g))))
...
(check-expect (next-tank (make-tank (- WIDTH TCUT) 1)) (make-tank (+ 285 TANK-SPEED) -1))
(check-expect (next-tank (make-tank TCUT 1)) (make-tank (+ 15 TANK-SPEED) 1))
...
;; Helper Function
;; integer -> integer
;; retuns how many ships destroyed by the player
(check-expect (next-score 50 (list I1 I2 I3) (list M1 M2)) 50)
(check-expect (next-score 50 (list I1 I2 I3) (list M1 M2 M3)) 60)

; (define (next-score n loi lom) 0)  ; stub

(define (next-score n loi lom)
  (cond [(empty? loi) n]
        [else
         (if (missile-hit? lom (first loi))
             (next-score (+ 10 n) (rest loi) lom)
             (next-score n (rest loi) lom))]))


;; Primary Function 
;; game -> Image
;; produces image of the game
(check-expect (render-game G0) (place-image (text (string-append "Score: " (number->string 0))
                                                  16 "purple")
                                            40 20 (render-invader G0)))

; (define (render-game G0) BACKGROUND)

(define (render-game g)
  (place-image (text (string-append "Score: " (number->string (game-score g)))
                     16 "purple") 40 20 (render-invader g)))

;; Helper Function
;; game -> image
;; produces image of all invaders in the list
(check-expect (render-invader G0) (place-images empty empty (render-missile-tank G0)))
(check-expect (render-invader G2) (place-images (list INVADER) (list-iposn (game-invaders G2)) (render-missile-tank G2)))
(check-expect (render-invader G3) (place-images (list INVADER INVADER) (list-iposn (game-invaders G3)) (render-missile-tank G3)))

; (define (render-invader g) BACKGROUND)  ; stub

(define (render-invader g)
  (place-images (list-img INVADER (length (game-invaders g)))
                (list-iposn (game-invaders g))
                (render-missile-tank g)))
...
;; game -> Image
;; produces image of all missile in the list and the tank
(check-expect (render-missile-tank G0) (place-images empty empty (place-image TANK 150 (- HEIGHT TANK-HEIGHT/2) BACKGROUND)))
(check-expect (render-missile-tank G2) (place-images (list MISSILE) (list-mposn (game-missiles G2))
                                                     (place-image TANK 50 (- HEIGHT TANK-HEIGHT/2) BACKGROUND)))
(check-expect (render-missile-tank G3) (place-images (list MISSILE MISSILE) (list-mposn (game-missiles G3))
                                                     (place-image TANK 50 (- HEIGHT TANK-HEIGHT/2) BACKGROUND)))

; (define (render-missile-tank g) BACKGROUND)  ; stub

(define (render-missile-tank g)
...
(check-expect (move-game (make-game empty empty (make-tank 150 1) 0) "left") (make-game empty empty (make-tank 150 -1) 0))
(check-expect (move-game (make-game (list I1) (list M1) (make-tank 150 -1) 0) "right")
              (make-game (list I1) (list M1) (make-tank 150 1) 0))
(check-expect (move-game (make-game (list I1 I2 I3) (list M1 M2) (make-tank 230 -1) 0) " ")
              (make-game
               (list I1 I2 I3) (list (make-missile 230 488) M1 M2) (make-tank 230 -1) 0))
(check-expect (move-game G2 "a") G2)

; (define (move-game g key) G0)   ; stub

(define (move-game g key)
  (cond [(key=? "left" key)
         (make-game (game-invaders g) (game-missiles g) (make-tank (tank-x (game-tank g)) -1) (game-score g))]
        [(key=? "right" key)
         (make-game (game-invaders g) (game-missiles g) (make-tank (tank-x (game-tank g)) 1) (game-score g))]
        [(key=? " " key)
         (make-game (game-invaders g) (add-missile g) (game-tank g) (game-score g))]
        [else g]))
...
; (define (add-missile G0) empty)   ; stub
...
;; game -> Image
;; produces game over on screen with current score 

(define (GAME-OVER g)
  (place-image (above (text "Game Over!" 48 "red")
                      (text (string-append "Your Score: "
